...
/**
 * JWT 검증 후 SecurityContext에 LoginUser 기반 Authentication을 설정.
 * promoterId, venueId 클레임도 파싱하여 LoginUser에 포함.
 */
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (token != null && jwtUtil.validateToken(token)) {
            ...
            // JJWT는 정수를 Integer로 역직렬화할 수 있으므로 Number로 안전하게 캐스팅
    ...
    /** JWT claim 값을 Long으로 안전하게 변환 (Integer/Long 모두 처리) */
    ...
    private String extractToken(HttpServletRequest request) {
        if (request.getCookies() != null) {
            return Arrays.stream(request.getCookies())
                    .filter(c -> "PARTNER_TOKEN".equals(c.getName()))
                    .map(Cookie::getValue)
