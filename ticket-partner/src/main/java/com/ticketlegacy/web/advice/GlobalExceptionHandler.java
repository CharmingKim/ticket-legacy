...
public class GlobalExceptionHandler {
    ...
    /** 중복 키 — 어떤 컬럼의 중복인지 구분하여 적절한 메시지 반환 */
    ...
    public ResponseEntity<ApiResponse<?>> handleDuplicateKey(DuplicateKeyException e) {
        ...
        if (msg.contains("uq_venue_section")) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(ApiResponse.error("이미 존재하는 구역명입니다. (같은 공연장에 동일한 구역명은 1개만 허용됩니다)"));
        }
        // 그 외 일반 중복
        return ResponseEntity.status(HttpStatus.CONFLICT)
    ...
    /** 잘못된 상태/인자 예외 (Admin 서비스 등에서 검증 실패 시) */
